...
- Standalone pages, **no `Base.astro` layout** (no site header/footer): `src/pages/links.astro` + `src/pages/{de,es,ro,uk,ru,fr,pt}/links.astro`. The 8 files are identical except `lang`/`path` and the translated strings in the frontmatter (title, description, tagline, 4 button labels), so edit all 8 together. Design "Halo" (ring-light logo, warm palette). Each locale is fully translated (brand name EWOMODELS is never translated); the language dropdown links to the matching `/<lang>/links`. The root `/links` redirects by browser language client-side, honouring the `ewo-lang-set` localStorage flag (same as the homepage), and keeps `?ref=`.
