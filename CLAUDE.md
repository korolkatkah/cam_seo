...
- Standalone pages, **no `Base.astro` layout** (no site header/footer): `src/pages/links.astro` + `src/pages/{ro,ru}/links.astro` (DE/ES/UK/FR/PT were dropped on 2026-09-22). The 3 files are identical except `lang`/`path` and the translated strings in the frontmatter (title, description, tagline, 4 button labels), so edit all 3 together. Design "Halo" (ring-light logo, warm palette). Each locale is fully translated (brand name EWOMODELS is never translated); the language dropdown links to the matching `/<lang>/links`. The first button (agency) points to `https://ewomodels.com/?utm_source=minilanding`. The root `/links` redirects by browser language client-side, honouring the `ewo-lang-set` localStorage flag (same as the homepage), and keeps `?ref=`.
